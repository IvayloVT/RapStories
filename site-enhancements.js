...
window.RAP_STORIES_BASE_URL = window.RAP_STORIES_BASE_URL
  ...
  function injectHeaderMenuStyles() {
    if (document.getElementById("headerMenuStyles")) return;
    const style = document.createElement("style");
    style.id = "headerMenuStyles";
    style.textContent = [
      "header#mainHeader .nav{display:grid!important;grid-template-columns:minmax(0,1fr)!important;justify-items:stretch!important;gap:14px;align-items:stretch;min-height:auto;padding:16px 0}",
      "header#mainHeader .header-top{display:grid!important;grid-template-columns:minmax(0,1fr) auto!important;align-items:center;justify-content:stretch!important;gap:16px;width:100%;margin:0}",
      "header#mainHeader .header-top .brand{display:flex;align-items:center;justify-self:start;align-self:center;min-width:0;margin:0}",
      "header#mainHeader .header-top .brand-text{white-space:nowrap}",
      "header#mainHeader .nav-toggle{display:inline-flex!important;align-items:center;justify-content:center;justify-self:end!important;align-self:center;min-height:48px;padding:12px 18px;border-radius:999px;border:1px solid rgba(255,255,255,0.12);background:rgba(255,255,255,0.05);color:var(--text,#f4f4f5);font:inherit;font-weight:800;letter-spacing:.2px;cursor:pointer;transition:transform .2s ease,border-color .2s ease,background .2s ease;box-shadow:0 16px 40px rgba(0,0,0,.2);margin:0 0 0 auto!important;flex:0 0 auto}",
      "header#mainHeader .nav-toggle:hover{transform:translateY(-2px);border-color:rgba(212,175,55,0.32);background:rgba(255,255,255,0.08)}",
      ...
      "header#mainHeader #site-nav .nav-actions .nav-btn,header#mainHeader #site-nav .nav-actions .theme-toggle{width:100%;justify-content:center}",
      "@media (min-width:821px){header#mainHeader .nav{display:flex!important;justify-content:space-between!important;align-items:center!important;gap:24px;min-height:82px;padding:0}header#mainHeader .header-top{display:flex!important;align-items:center;justify-content:flex-start!important;gap:0;width:auto}header#mainHeader .nav-toggle{display:none!important}header#mainHeader #site-nav{display:block!important;width:auto}header#mainHeader #site-nav .nav-panel{display:flex;align-items:center;gap:24px;padding:0;border-radius:0;background:transparent;border:none;box-shadow:none}header#mainHeader #site-nav .nav-links{display:flex!important;align-items:center;gap:24px;width:auto}header#mainHeader #site-nav .nav-links a{position:relative;display:inline-flex!important;align-items:center;justify-content:flex-start;padding:0;border-radius:0;background:transparent;border:none;min-height:auto;color:var(--muted,#a5a5b1);text-align:left}header#mainHeader #site-nav .nav-links a:hover,header#mainHeader #site-nav .nav-links a.active{color:var(--text,#f4f4f5);background:transparent;border-color:transparent}header#mainHeader #site-nav .nav-links a::after{content:'';display:block!important;position:absolute;left:0;bottom:-8px;width:100%;height:2px;border-radius:999px;background:linear-gradient(135deg,#d4af37,#fff0a5);transform:scaleX(0);transform-origin:left;transition:transform .25s ease}header#mainHeader #site-nav .nav-links a:hover::after,header#mainHeader #site-nav .nav-links a.active::after{transform:scaleX(1)}header#mainHeader #site-nav .nav-actions{display:flex;align-items:center;gap:12px;width:auto}header#mainHeader #site-nav .nav-actions .nav-btn,header#mainHeader #site-nav .nav-actions .theme-toggle{width:auto;justify-content:center}}",
  ...
  function updateHeaderOffset(header) {
    if (!header) return;
  ...
  function setupHeaderMenu() {
    ...
    let brand = navShell.querySelector(".brand");
    let navLinks = navShell.querySelector(".nav-links");
    if (!brand || !navLinks) return;

    let headerTop = navShell.querySelector(".header-top");
    if (!headerTop) {
      headerTop = document.createElement("div");
      headerTop.className = "header-top";
      navShell.insertBefore(headerTop, navShell.firstChild || null);
    }

    if (brand.parentNode !== headerTop) {
      headerTop.insertBefore(brand, headerTop.firstChild || null);
    }

    let siteNav = navShell.querySelector("#site-nav");
    if (!siteNav) {
      if (navLinks.tagName.toLowerCase() === "nav") {
        siteNav = navLinks;
        siteNav.id = "site-nav";
        siteNav.classList.remove("nav-links");
        const linksWrapper = document.createElement("div");
        linksWrapper.className = "nav-links";
        while (siteNav.firstChild) {
          linksWrapper.appendChild(siteNav.firstChild);
        }
        siteNav.appendChild(linksWrapper);
        navLinks = linksWrapper;
      } else {
        siteNav = document.createElement("nav");
        siteNav.id = "site-nav";
        navShell.appendChild(siteNav);
      }
    }

    let panel = siteNav.querySelector(".nav-panel");
    if (!panel) {
      panel = document.createElement("div");
      panel.className = "nav-panel";
      while (siteNav.firstChild) {
        panel.appendChild(siteNav.firstChild);
      }
      siteNav.appendChild(panel);
    }

    navLinks = siteNav.querySelector(".nav-links") || navLinks;
    if (navLinks && navLinks.parentNode !== panel) {
      panel.insertBefore(navLinks, panel.firstChild || null);
    }

    let navToggle = navShell.querySelector(".nav-toggle");
    if (!navToggle) {
    ...
    if (navToggle.parentNode !== headerTop) {
      headerTop.appendChild(navToggle);
    }

    let actions = siteNav.querySelector(".nav-actions") || navShell.querySelector(".nav-actions");
    if (!actions) {
      actions = document.createElement("div");
      actions.className = "nav-actions";
    }
    if (actions.parentNode !== panel) {
      panel.appendChild(actions);
    }

    const cta = navShell.querySelector(".nav-btn");
    ...
    if (!header.dataset.menuDismissBound) {
      ...
      header.dataset.menuDismissBound = "true";
    }
  ...
  function setupThemeToggle() {
    ...
    const menuRoot = nav.querySelector("#site-nav") || nav;
    let actions = menuRoot.querySelector(".nav-actions");
    const cta = menuRoot.querySelector(".nav-btn") || nav.querySelector(".nav-btn");
    if (!actions) {
      actions = document.createElement("div");
      actions.className = "nav-actions";
      if (cta) {
